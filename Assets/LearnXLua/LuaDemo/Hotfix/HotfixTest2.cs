...
namespace LearnXLua.LuaDemo.Hotfix {
	...
	public class HotfixTest2 : MonoBehaviour {
		void Start() {
			...
			var d1 = (DateTime.Now - start).TotalMilliseconds;
			...
			var d2 = (DateTime.Now - start).TotalMilliseconds;
			...
			//language=Lua
			luaEnv.DoString(@"
            xlua.hotfix(CS.LearnXLua.LuaDemo.Hotfix.HotfixCalc, 'Add', function(self, a, b)
                return a + b
            end)
        ");
			...
			//language=Lua
			luaEnv.DoString(@"
            xlua.hotfix(CS.LearnXLua.LuaDemo.Hotfix.HotfixCalc, 'TestOut', function(self, a, c, go)
        ...
        ");
			...
			//language=Lua
			luaEnv.DoString(@"
            xlua.hotfix(CS.LearnXLua.LuaDemo.Hotfix.HotfixCalc, {
                 Test1 = function(self)
                    print('Test1', self)
                    return 1
                 end;
        ...
        ");
			...
			xlua.hotfix(CS.LearnXLua.LuaDemo.Hotfix.StatefullTest, {
				['.ctor'] = function(csObj)
			...
			//language=Lua
			luaEnv.DoString(@"
            xlua.hotfix(CS.LearnXLua.LuaDemo.Hotfix.GenericClass(CS.System.Double), {
                ['.ctor'] = function(obj, a)
			...
			//language=Lua
			luaEnv.DoString(@"
            xlua.hotfix(CS.LearnXLua.LuaDemo.Hotfix.InnerTypeTest, 'Bar', function(obj)
                print('lua Bar', obj)
                return {x = 10, y = 20}
            end)
			");
			...
			//language=Lua
			luaEnv.DoString(@"
            xlua.hotfix(CS.LearnXLua.LuaDemo.Hotfix.StructTest, 'GetGo', function(self, a, b)
			...
			//language=Lua
			luaEnv.DoString(@"
            xlua.hotfix(CS.LearnXLua.LuaDemo.Hotfix.GenericStruct(CS.System.Int32), 'GetA', function(self, a)
                print('GetA',self, a)
                return 789
			end)
			");
			...
			//language=Lua
			luaEnv.DoString(@"
            xlua.hotfix(CS.LearnXLua.LuaDemo.Hotfix.BaseTest, 'Foo', function(self, p)
                    print('BaseTest', p)
                end)
            xlua.hotfix(CS.LearnXLua.LuaDemo.Hotfix.BaseTest, 'ToString', function(self)
		...
		void TestStateful() {
			...
			Action<int, double> cb = (a, b) => { Debug.Log("a:" + a + ",b:" + b); };
			sft.AEvent += cb;
			sft.Start();
			sft.Start();
			sft.AEvent -= cb;
