	...
	public class HotfixTest : MonoBehaviour {
		...
		void OnGUI() {
			if(GUI.Button(new Rect(10, 10, 300, 80), "Hotfix")) {
				//language=Lua
				luaEnv.DoString(@"
                xlua.hotfix(CS.LearnXLua.LuaDemo.Hotfix.HotfixTest, 'Update', function(self)
			...
			const string chHint = @"在运行该示例之前，请细致阅读xLua文档，并执行以下步骤：
...
3.注入：执行 'XLua > Hotfix Inject In Editor' 菜单。注入成功会打印 'hotfix inject finish!' 或者 'had injected!' 。";
			...
			const string enHint = @"Read documents carefully before you run this example, then follow the steps below:
